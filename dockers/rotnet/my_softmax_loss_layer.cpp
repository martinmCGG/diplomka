...
void MySoftmaxWithLossLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  LOG(INFO) << "MySoftmaxWithLossLayer<>::LayerSetUp";
  ...
  Blob<Dtype> bottomR( num * nRotation, nClassLabel, bottom[0]->height(), bottom[0]->width() );
...
template <typename Dtype>
void MySoftmaxWithLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
// template <typename Dtype>
// Dtype MySoftmaxWithLossLayer<Dtype>::get_normalizer(
...
template <typename Dtype>
void MySoftmaxWithLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  if( (num % nRotation) != 0 ){
    LOG(INFO) << "num should be multiples of nRotation. " << num << " != " << nRotation << " * N.";
    exit(1);
  }
  nSample = num / nRotation;
  max_ang.resize( nSample );

  for (int n = 0; n < nSample; ++n) {
    const int label_ = static_cast<int>(label[ n * nRotation ]);
    max_ang[ n ] = -1;
    Dtype max_prob = - Dtype(FLT_MAX);
    //LOG(INFO) << "max_prob init: " << max_prob;
	...
	for (int j = 0; j < nRotation; ++j) {
	  ...
	  tmp_prob += log( std::max(prob_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + label_ ], Dtype(FLT_MIN)) );
	  tmp_prob -= log( std::max(prob_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
	}
	if( tmp_prob > max_prob ){ // sometimes this is not valid for any (i,j), max_arg stays -1
	  max_ang[ n ] = i;
	  max_prob = tmp_prob;
	}
      }
      if (max_ang[n] == -1) {
        LOG(WARNING) << "max_ang[" << n << "] stayed -1 (using_upright)";
        max_ang[n] = 0;
      }
	...
	Dtype tmp_prob = 0;
	for (int j = 0; j < nRotation; ++j) {
	  tmp_prob += log( std::max(prob_data[ ( n * nRotation + ang[ i ][ j ] ) * dim + j * nClassLabel + label_ ], Dtype(FLT_MIN)) );
	  tmp_prob -= log( std::max(prob_data[ ( n * nRotation + ang[ i ][ j ] ) * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
	}
	if( tmp_prob > max_prob ){
      ...
      if (max_ang[n] == -1) {
        LOG(WARNING) << "max_ang[" << n << "] stayed -1 (not using_upright)";
        max_ang[n] = 0;
      }
      for (int i = 0; i < nRotation; ++i) 
	labelR[ n * nRotation + ang[ max_ang[ n ] ][ i ] ] = i * nClassLabel + label_;
    }
    //LOG(INFO) << "max_prob end: " << max_prob;
  ...
  Dtype loss = 0;
  for (int i = 0; i < num; ++i) {
    loss += -log(std::max(prob_data[i * dim + labelR[i] ],
                     Dtype(FLT_MIN)));
    int id = labelR[i] / nClassLabel;
    for (int j = 0; j < nRotation; ++j) 
      if( j != id )
	loss += -log(std::max(prob_data[i * dim + j * nClassLabel + nClassLabel - 1 ],
			 Dtype(FLT_MIN)));
  }
  top[0]->mutable_cpu_data()[0] = loss / (num * nRotation);
}

template <typename Dtype>
void MySoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(MySoftmaxWithLossLayer);
#endif

INSTANTIATE_CLASS(MySoftmaxWithLossLayer);
