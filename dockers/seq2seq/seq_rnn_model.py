...
class SequenceRNNModel(object):
    ...
    def step(self, session, encoder_inputs, decoder_inputs, target_weights, forward_only=False):
        ...
        if not forward_only:
            output_ops = [self.optimizer, self.cost, self.logits]
        else:
            #output_ops = [self.logits, self.encoder_hidden_state]
            output_ops = [self.logits, self.cost]
        outputs = session.run(output_ops, input_feed)
        if not forward_only:
            return outputs[1], outputs[2] #Gradient, loss, no outputs, no encoder_hidden
        else:
            #attns_weights = session.run(self.attns_weights, input_feed)
            return outputs[0], outputs[1] #No gradient, no loss, outputs logits, encoder_hidden
